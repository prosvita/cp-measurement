...
class DataSources {
    constructor (sources) {
        ...
        for (const source of sources) {
            ...
            this.sources[source.name].loadTime = new Date()
        }
    }
    ...
    counter (from, to, interval, t, datapoints, inc = true) {
        if (t > (from - interval) && t <= to) {
            ...
            datapoints[i][0] += inc ? 1 : -1
        }
    }

    reload (name) {
        const time = new Date()
        if (this.sources[name].loadTime < (time - RELOAD_TIMEOUT)) {
            this.sources[name].loadTime = time
            this.sources[name].db.read()
        }
    }
